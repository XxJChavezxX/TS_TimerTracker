...
public class EditTimeActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
			...
			land = true;
			/**
			 * Sending ID back to the Timeloglist fragment if this activity is
			 * in landscape mode
			 **/
			if (id > 0) {
				/**
				 * Share id with other activities or fragments by using
				 * sharedPref method
				 * **/
				SharedPreferences sharedPref = getApplicationContext()
						.getSharedPreferences(
								getString(R.string.pref_data_key), 0);
				SharedPreferences.Editor editor = sharedPref.edit();
				editor.putString(getString(R.string.timelog_id),
						String.valueOf(id)).commit();
				// editor.commit();
			}
			// finish();
			// return;
		}// checking for the Orientation must be before defining the content
			// view
		else
			land = false;
		...
		if (timelog != null) {
			for (int i = 0; i < companies.size(); i++) {
				...
				String dtoCompName = logic.getCompanyById(
						timelog.getCompanyId()).getName();
		...
		if (timelog != null) {
			...
			// clockintv.setText("Clock In Date: " +
			// timelog.getStartTime().substring(0,10));
			// clockouttv = (TextView) findViewById(R.id.clockouttv);
			// clockouttv.setText("Clock Out Date: " +
			// timelog.getEndTime().substring(0,10));
			...
			// c.getTime();
			// StartPicker.setCurrentHour(c.get(Calendar.HOUR_OF_DAY)); // sets
			// hour with am or pm time
			// StartPicker.setCurrentMinute(c.get(Calendar.MINUTE));
			// Calendar cl = TimeHelper.setCalendar(timelog.getEndTime());
			// cl.getTime();
			// EndPicker.setCurrentHour(cl.get(Calendar.HOUR_OF_DAY));// sets
			// hour with am or pm time
		...
		if (land) {
			StartDateTime = StartDatetxt.getText().toString() + " "
					+ Starttxt.getText().toString();
			EndDateTime = EndDatetxt.getText().toString() + " "
		...
		SaveBttn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (timelog != null) {
					newStartTime = (StartDate == null || StartDate.equals("")) ? timelog
							.getStartTime() : StartDate; // Get the starttime
															// from timepicker
					newEndTime = (EndDate == null || EndDate.equals("")) ? timelog
							.getEndTime() : EndDate; // Get the endtime from
														// timepicker

					StartDate = "";
					EndDate = "";
				}
				long longDate1 = TimeHelper.getLongFromDate(StartDatetxt
						.getText().toString());
				long longDate2 = TimeHelper.getLongFromDate(EndDatetxt
						.getText().toString());
		...
		CompSpinner
				.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
					...
					public void onItemSelected(AdapterView<?> parent,
						...
						// Toast.makeText(getApplicationContext(), "CompanyId: "
			...
			public void onClick(View v) {
				...
				if (StartDate == null || StartDate.equals("")) {
					if (timelog != null)
						...
						c = TimeHelper.setCalendar(StartDatetxt.getText()
								.toString()
								+ " "
								+ Starttxt.getText().toString());
				...
				TimePickerDialog tdp = new TimePickerDialog(
						EditTimeActivity.this, new OnTimeSetListener() {
			...
			public void onClick(View v) {
				...
				if (EndDate == null || EndDate.equals("")) {
					if (timelog != null)
						...
						c = TimeHelper
								.setCalendar(EndDatetxt.getText().toString()
										+ " " + Endtxt.getText().toString());
				} else
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_settings:
			Toast.makeText(this, "Settings was selected", Toast.LENGTH_LONG)
					.show();
			break;
		case R.id.action_delete:
							...
							if (logic.Error != null) {
								if (logic.Error.contains("key constraint")) {
									Toast.makeText(
											...
											"This record is being used somewhere else and cannot be deleted",
											Toast.LENGTH_LONG).show();
								} else
			...
			// Toast.makeText(this, "Record has been deleted successfully",
			// Toast.LENGTH_LONG).show();
			break;
	...
	private void validateTimes() {
		...
		if (timelog != null) {
			...
			if (timelog.getStartTime().equals(newStartTime)
					&& timelog.getEndTime().equals(newEndTime)
					&& timelog.getCompanyId() == _id) {
				// Toast.makeText(this,
				// "Times were not updated because no change was made",
				// Toast.LENGTH_LONG).show();
				finish();
			} else {
				...
				if (lg1 < lg2) {
					...
					Toast.makeText(this, "Timelog was updated successfully!",
							Toast.LENGTH_LONG).show();
					...
					Toast.makeText(
							this,
							"Start Time cannot be less or the same as the End Time, please try again",
							Toast.LENGTH_LONG).show();
			}
		...
		// /***** creates a new timelog ******///
			...
			long lg1 = TimeHelper.getLongFromHour(StartDatetxt.getText()
					.toString() + " " + Starttxt.getText().toString());
			long lg2 = TimeHelper.getLongFromHour(EndDatetxt.getText()
					.toString() + " " + Endtxt.getText().toString());

			if (lg1 < lg2) {
				SaveNewTimeLog();
				Toast.makeText(this, "Timelog was created successfully!",
						Toast.LENGTH_LONG).show();
	...
	private void SaveNewTimeLog() {
		...
		timelog = new TimeLogDTO();
		timelog.setDate(StartDatetxt.getText().toString()); // saves the date
															// format as string
